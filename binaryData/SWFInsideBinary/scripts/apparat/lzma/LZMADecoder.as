package apparat.lzma
{
   ...
   public final class LZMADecoder
   {
         ...
         while(param3 < 0 || _loc5_ < param3)
         {
            ...
            else
            {
               ...
               else
               {
                  ...
                  if((_loc15_ = this._posSlotDecoder[Base.getLenToPosState(_loc13_)].decode(this._rangeDecoder)) >= Base.kStartPosModelIndex)
                  {
                     _loc16_ = (_loc15_ >> 1) - 1;
                     ...
                     else if((_loc7_ = int((_loc7_ += this._rangeDecoder.decodeDirectBits(_loc16_ - Base.kNumAlignBits) << Base.kNumAlignBits) + this._posAlignDecoder.reverseDecode(this._rangeDecoder))) < 0)
                     {
                        if(_loc7_ == -1)
                        {
                           break;
      ...
      public function setDecoderProperties(param1:Vector.<int>) : Boolean
      {
         ...
         var _loc4_:int;
         var _loc5_:int = (_loc4_ = _loc2_ / 9) % 5;
         var _loc6_:int = _loc4_ / 5;
         var _loc7_:int = 0;
         var _loc8_:int = 0;
         while(_loc8_ < 4)
         {
            _loc7_ += (param1[int(1 + _loc8_)] & 255) << _loc8_ * 8;
            _loc8_++;
         }
