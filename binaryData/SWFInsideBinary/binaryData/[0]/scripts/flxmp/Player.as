   ...
   public class Player
   {
      ...
      private function nextRow() : void
      {
         ...
         while(_loc1_ < this.mod.numChannels)
         {
            ...
            _loc2_ = int(this.pattern.position);
            ...
            _loc3_ = int(this.pattern.readUnsignedByte());
      ...
      private function perTickProcessing() : void
      {
         ...
         while(_loc1_ < this.mod.numChannels)
         {
            ...
            {
               if((this.chan.volumeCommand & 0xF0) == 96)
               {
                  if((this.chan.volumeTabIndex = this.chan.volumeTabIndex - (this.chan.volumeCommand & 0x0F)) < 0)
                  {
                     this.chan.volumeTabIndex = 0;
                  }
                  this.chan.columnVolume = this.VOL_TAB_LIN[this.chan.volumeTabIndex];
               }
               else if((this.chan.volumeCommand & 0xF0) == 112)
               {
                  if((this.chan.volumeTabIndex = this.chan.volumeTabIndex + (this.chan.volumeCommand & 0x0F)) > 64)
                  {
                     this.chan.volumeTabIndex = 64;
                  }
                  this.chan.columnVolume = this.VOL_TAB_LIN[this.chan.volumeTabIndex];
               }
               else if((this.chan.volumeCommand & 0xF0) == 128)
               {
                  if(this.tickCnt == 0)
                  {
                     if((this.chan.volumeTabIndex = this.chan.volumeTabIndex - (this.chan.volumeCommand & 0x0F)) < 0)
               ...
               else if((this.chan.volumeCommand & 0xF0) == 144)
               {
                  if(this.tickCnt == 0)
                  {
                     if((this.chan.volumeTabIndex = this.chan.volumeTabIndex + (this.chan.volumeCommand & 0x0F)) > 64)
               ...
               else if((this.chan.volumeCommand & 0xF0) != 160)
               {
                  if((this.chan.volumeCommand & 0xF0) != 176)
                  {
                     if((this.chan.volumeCommand & 0xF0) == 192)
                     {
                        this.chan.wavePanning = Number(this.chan.volumeCommand & 0x0F) * 0.0625;
                     }
                     else if((this.chan.volumeCommand & 0xF0) != 208)
                     {
                        if((this.chan.volumeCommand & 0xF0) != 224)
                        {
                           if((this.chan.volumeCommand & 0xF0) == 240)
                           {
                           }
                        }
                     }
            ...
            if(this.chan.effect != 0)
            {
               ...
               else if(this.chan.effect == 4)
               {
                  ...
                  this.chan.vibrate = Number((this.chan.parameter & 0xF0) >> 4);
                  this.chan.vibdepth = (this.chan.parameter & 0x0F) << 3;
                        ...
                        else if(this.chan.effect != 10)
                        {
                           if(this.chan.effect != 11)
                           {
                              if(this.chan.effect != 12)
                              {
                                 ...
                                 {
                                    _loc2_ = this.chan.parameter & 0xF0;
                                    if(_loc2_ != 16)
                                    {
                                       if(_loc2_ != 32)
                                       {
                                                      ...
                                                      if(_loc2_ != 112)
                                                      {
                                                         if(_loc2_ != 144)
                                                         {
                                                            ...
                                                            else if(_loc2_ != 176)
                                                            {
                                                               if(_loc2_ == 192)
                                                               {
                                                                  if(this.tickCnt >= (this.chan.parameter & 0x0F))
                                                                  {
                                                                     this.chan.targetVolume = 0;
                                                                     this.chan.columnVolume = 0;
                                                                  }
                                 ...
                                 else if(this.chan.effect != 21)
                                 {
                                    if(this.chan.effect != 25)
                                    {
                                       if(this.chan.effect == 27)
                                       {
                                          ...
                                          if(this.tickCnt % (this.chan.parameter & 0x0F) == 0)
                                          {
                                             this.chan.wavePos = 0;
                                          }
