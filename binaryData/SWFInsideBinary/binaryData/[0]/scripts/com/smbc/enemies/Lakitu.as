package com.smbc.enemies
{
   ...
   public class Lakitu extends Enemy
   {
      ...
      public function Lakitu(param1:LakituSpawner, param2:Boolean)
      {
         super();
         this.lakSpwnr = param1;
         ...
         this.edgeBuffer = com.smbc.§main:LevObj§.TILE_SIZE * 2;
         y = GLOB_STG_TOP + com.smbc.§main:LevObj§.TILE_SIZE * 3;
         if(param2)
         {
            y = MIDDLE_Y;
         }
      ...
      override protected function checkState() : void
      {
         ...
         switch(GameSettings.mapDifficulty)
         {
            case MapDifficulty.EASY:
               this.maxSpinyDifficulty = 2;
               break;
            case MapDifficulty.NORMAL:
               this.maxSpinyDifficulty = 4;
               break;
            case MapDifficulty.HARD:
               this.maxSpinyDifficulty = 6;
         }
         if(cState == "wait" && this.SPINEY_DCT.length < this.maxSpinyDifficulty)
         {
            this.throwSpiney();
         }
         super.checkState();
      ...
      private function throwTmrLsr(param1:TimerEvent) : void
      {
         ...
         {
            this.throwTmr.reset();
            if(this.SPINEY_DCT.length < this.maxSpinyDifficulty)
            {
               this.throwSpiney();
            }
            else
      ...
      override protected function removeListeners() : void
      {
         ...
         if(Boolean(this.throwTmr) && this.throwTmr.hasEventListener(TimerEvent.TIMER_COMPLETE))
         {
            this.throwTmr.removeEventListener(TimerEvent.TIMER_COMPLETE,this.throwTmrLsr);
         }
         if(Boolean(this.hideTmr) && this.hideTmr.hasEventListener(TimerEvent.TIMER_COMPLETE))
         {
