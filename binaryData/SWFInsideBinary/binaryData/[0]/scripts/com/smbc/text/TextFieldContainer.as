package com.smbc.text
{
   ...
   public class TextFieldContainer extends Sprite
   {
      ...
      private function recolorCharacters() : void
      {
         ...
         if(_loc2_)
         {
            _loc4_ = _loc3_.getPaletteFromRow(_loc2_,1,PaletteSheet.THEME_TYPE_INTERFACE,true);
            _loc5_ = _loc4_.clone();
         }
         if(_loc4_ && GM.shouldBeRecoloredToGb(GM.drawingBoardInterfaceSkinCont) && Boolean(GameSettings.getInterfacePaletteLimited()))
         {
         ...
         while(_loc8_ < _loc6_)
         {
            ...
            _loc9_ = _loc1_.getChildAt(0) as Bitmap;
            _loc10_ = _loc9_.bitmapData.clone();
      ...
      public function center() : void
      {
         ...
         var _loc10_:int = 0;
         var _loc1_:int = int(this.lineArr.length);
         var _loc2_:Number = this.charContainer.width;
         var _loc3_:int = 0;
         while(_loc3_ < _loc1_)
         {
            _loc4_ = this.lineArr[_loc3_];
            if(_loc4_)
            {
               _loc5_ = int(_loc4_.length);
               _loc6_ = this.getMinX(_loc4_);
               _loc7_ = this.getMaxX(_loc4_);
