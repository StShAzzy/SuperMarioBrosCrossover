package com.smbc.managers
{
   ...
   public final class SoundManager extends MainManager
   {
      ...
      public var sfxVolMinusNum:Number = 45;
      
      public var musicVolMinusNum:Number = 0;
      ...
      public function resumeBgm() : void
      {
         ...
         if(this.bgmPausePos >= 0 && Boolean(this.bgmClassName))
         {
      ...
      public function pauseLoopingsSfx() : void
      {
         ...
         {
            if(_loc1_.soundName == SoundNames.SFX_LINK_BOOMERANG || _loc1_.soundName == SoundNames.SFX_SIMON_CROSS || _loc1_.soundName == SoundNames.SFX_SIMON_AXE || _loc1_.soundName == SoundNames.SFX_MEGA_MAN_CHARGE_START)
            {
               _loc1_.pauseSound();
            }
         }
      ...
      public function playEmulatedSfx(param1:Array) : ByteArray
      {
         ...
         var _loc4_:ByteArray = param1[_loc3_][IND_MUSIC_BUFFER];
         if(_loc4_)
         {
            return _loc4_;
         }
      ...
      public function playMusic(param1:*, param2:GameMusicEmu = null, param3:Boolean = false, param4:ByteArray = null, param5:int = -1, param6:Boolean = false) : void
      {
         ...
         if(_loc8_)
         {
            _loc13_ = int(_loc8_[this.IND_MUSIC_START_TIME]);
         }
         else
         {
      ...
      public function playStoredSounds(param1:CustomDictionary) : void
      {
         ...
         for each(_loc2_ in param1)
         {
            ...
            _loc4_ = this.CUR_SNDS_DCT[_loc2_];
            if(_loc4_)
            {
               this.removeSnd(_loc4_);
            }
            _loc5_ = SoundContainer;
            _loc8_ = this.getMusicArr(_loc2_);
            if(_loc8_)
            {
               ...
               _loc7_ = this.playEmulatedSfx(_loc8_);
               if(_loc7_)
               {
      ...
      public function changeMusic(param1:String = null, param2:Boolean = false) : void
      {
         ...
         if(!param1)
         {
            ...
            _loc4_ = true;
            if(Boolean(player) && player.starPwrBgmShouldBePlaying)
            {
               _loc9_ = _loc11_.getSongFromTypePlayList(MusicType.STAR);
               if(!_loc9_)
               {
                  _loc11_ = grMngr.drawingBoardMapSkinCont.games[_loc10_];
                  _loc9_ = _loc11_.getSongFromTypePlayList(MusicType.STAR);
               }
            ...
            {
               param1 = level.type;
               _loc12_ = LevelDataManager.currentLevelData.getMainArea(level.id);
         ...
         {
            _loc13_ = MegaManBase.getProtoManWhistleSong();
            if(_loc13_)
            {
               _loc9_ = _loc13_;
            }
         ...
         if(_loc9_)
         {
            if(!_loc3_.isPlaying || this.songCurrentlyPlaying != _loc9_ || Boolean(_loc3_.fadeTmr))
            {
      ...
      public function getMusicCharName() : String
      {
         ...
         {
            _loc2_ = int(grMngr.CLEAN_BMD_VEC_MAP[grMngr.cMapNum].charArr[0]);
         }
      ...
      {
         if(Boolean(level) && level is TitleLevel)
         {
            return;
         }
         if(Boolean(this._bgm) && this._bgm.soundName == this.SN_SECONDS_LEFT_INTRO)
         {
            return;
         }
         this.changeMusic();
