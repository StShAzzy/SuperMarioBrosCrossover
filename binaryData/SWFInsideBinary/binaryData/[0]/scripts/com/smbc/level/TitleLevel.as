package com.smbc.level
{
   ...
   public class TitleLevel extends Level implements IKeyPressable
   {
      ...
      public function TitleLevel()
      {
         ...
         {
            _loc5_ = int(_loc4_[IND_CHAR_NUM]);
            _loc6_ = _loc4_[IND_UPGRADES];
      ...
      override public function initiateLevel() : void
      {
         ...
         this.versionTfc.text = "Ver " + Versions.toString(GameSettings.VERSION_NUMBER);
         ...
         for each(_loc2_ in this.dataDct)
         {
            ...
            _loc4_ = _loc2_[IND_CHAR_CLASS] as Class;
            (_loc5_ = new _loc4_()).x = _loc3_.x;
            _loc5_.y = _loc3_.y;
      ...
      override protected function gameLoop(param1:TimerEvent) : void
      {
         ...
         for each(_loc2_ in this.dataDct)
         {
            ...
            if(locStgRht + 20 > _loc3_.x)
            {
               ...
               _loc5_ = charDct[_loc2_[IND_CHAR_NUM]];
               if(_loc5_)
               {
                  _loc5_.x = _loc4_.x;
                  _loc5_.y = _loc4_.y;
                  _loc5_.stopUpdate = false;
                  _loc6_ = new CharacterSequencer();
                  _loc6_.startNewSequence(_loc2_[IND_SEQ],_loc5_);
      ...
      private function checkForJump() : void
      {
         ...
         if(player.onGround || waterLevel && player.vy >= 0)
         {
            ...
            if(Boolean(getGroundAt(_loc4_ + TILE_SIZE,_loc3_ - TILE_SIZE)) || Boolean(getGroundAt(_loc4_,_loc3_ - TILE_SIZE)))
            {
               player.pressJmpBtn();
               return;
            }
