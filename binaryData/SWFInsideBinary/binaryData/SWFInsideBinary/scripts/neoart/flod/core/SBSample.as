package neoart.flod.core
{
   ...
   public class SBSample
   {
      ...
      public function store(param1:ByteArray) : void
      {
         ...
         if(this.bits == 8)
         {
            if((_loc5_ = param1.position + _loc4_) > param1.length)
            {
               _loc4_ = int(param1.length - param1.position);
            }
            _loc3_ = 0;
            while(_loc3_ < _loc4_)
            {
               if((_loc6_ = param1.readByte() + _loc2_) > 127)
               {
                  _loc6_ -= 256;
               }
               else if(_loc6_ < -128)
         ...
         {
            if((_loc5_ = param1.position + (_loc4_ << 1)) > param1.length)
            {
            ...
            while(_loc3_ < _loc4_)
            {
               if((_loc6_ = param1.readShort() + _loc2_) > 32767)
               {
                  _loc6_ -= 65536;
               }
