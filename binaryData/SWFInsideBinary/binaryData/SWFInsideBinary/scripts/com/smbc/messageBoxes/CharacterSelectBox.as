package com.smbc.messageBoxes
{
   ...
   public class CharacterSelectBox extends MessageBox implements ISingleton, IKeyPressable
   {
      ...
      public function CharacterSelectBox(param1:Boolean = false)
      {
      ...
      override protected function setUpText() : void
      {
         ...
         var _loc1_:int = this.ORDER_VEC.length;
         while(_loc3_ < _loc1_)
         {
      ...
      private function alignPortraits(param1:Boolean = false) : void
      {
         ...
         var _loc2_:int = this.PORT_VEC.length;
         var _loc4_:Vector.<CharacterPortrait> = new Vector.<CharacterPortrait>();
         var _loc5_:int = 0;
         while(_loc5_ < _loc2_)
         {
            _loc10_ = this.PORT_VEC[_loc5_];
            if(this.SELECTABLE_CHARS_VEC.indexOf(_loc5_) != -1 || _loc5_ == this.randomPortInd)
            {
      ...
      private function changeMode(param1:String) : void
      {
         ...
         if(this.mode == MODE_CHARACTER_SELECT)
         {
            ...
            while(_loc5_ < _loc4_)
            {
               if((_loc6_ = _loc3_[_loc5_]) != PS_FALLEN)
               {
         ...
         else if(this.mode == MODE_REVIVE)
         {
            ...
            while(_loc5_ < _loc4_)
            {
               ...
               if(!STAT_MNGR.charIsFullyUpgraded(_loc5_))
               {
                  this.addSelectableChar(this.convCharNumToInd(_loc5_),true);
               }
               _loc5_++;
            ...
            _loc4_ = this.SELECTABLE_CHARS_VEC.length;
            _loc5_ = 0;
            while(_loc5_ < _loc4_)
            {
               _loc8_ = this.SELECTABLE_CHARS_VEC[_loc5_];
               if((_loc6_ = _loc3_[this.convIndToCharNum(_loc8_)]) == PS_FALLEN)
               {
      ...
      private function setAllIconVisibility(param1:Boolean) : void
      {
         ...
         while(_loc3_ < _loc2_)
         {
            (_loc4_ = this.PORT_VEC[_loc3_]).setIconVisibility(param1);
            _loc3_++;
         }
      }
      ...
      private function reviveCharacter(param1:int) : void
      {
         ...
         if(_loc3_ != PS_FALLEN)
         {
            ...
            Character.hitRandomUpgrade(_loc2_,false);
            _loc5_ = true;
            if(this.CS_SCRN && this.CS_SCRN.player)
            {
         ...
         {
            if((_loc7_ = this.SELECTABLE_CHARS_VEC.indexOf(param1)) != -1)
            {
      ...
      private function changeNameBoxText() : void
      {
         ...
         if(this.mode == MODE_REVIVE)
         {
            ...
            if((_loc5_ = STAT_MNGR.numLives) > 1)
            {
      ...
      private function getRandomCharInd(param1:Boolean = true) : int
      {
         var _loc2_:int = 0;
         if(param1)
         {
            _loc2_ = int(Math.random() * (this.SELECTABLE_CHARS_VEC.length - 1));
            return this.SELECTABLE_CHARS_VEC[_loc2_];
         }
         return this.convCharNumToInd(STAT_MNGR.getRandomCharNum());
      }
      ...
      private function addSelectableChar(param1:int, param2:Boolean = false) : void
      {
         ...
         var _loc3_:int = this.SELECTABLE_CHARS_VEC.length;
         var _loc4_:Array;
         (_loc4_ = this.selectableCharsRowArr[this.getIndRowNum(param1)]).push(this.convGlobalIndToRowInd(param1));
      ...
      private function removeSelectableChar(param1:int, param2:Boolean = false) : void
      {
         ...
         var _loc4_:int = _loc3_.indexOf(this.convGlobalIndToRowInd(param1));
      ...
      override public function pressAtkBtn() : void
      {
         ...
         if(!(this.mode == MODE_CHARACTER_SELECT && this.cSelInd != this.randomPortInd))
         {
            if(this.mode == MODE_REVIVE && !STAT_MNGR.checkPStateVecForAllZeroes())
            {
      ...
      override public function pressJmpBtn() : void
      {
         if(this.mode == MODE_CHARACTER_SELECT)
         {
            ...
            {
               this.cSelInd = this.getRandomCharInd(true);
               this.setSelection(this.cSelInd,false);
               if(this.CS_SCRN)
               {
                  GraphicsManager.INSTANCE.changeCharacterSkin(this.CS_SCRN.player,STAT_MNGR.getRandomCharSkin(this.cCharNum));
                  this.CS_SCRN.chooseCharacter(this.cCharNum);
               }
            }
            else
            {
               nextMsgBxToCreate = new CharacterSkinMenu(this.cCharNum);
               SND_MNGR.playSound(SoundNames.SFX_GAME_COIN);
            }
            cancel();
            this.nameBox.cancel();
         ...
         else if(this.mode == MODE_REVIVE)
         {
            if(this.cSelInd == this.randomPortInd)
            {
               this.reviveCharacter(this.getRandomCharInd(true));
            }
            else
            {
      ...
      private function getIndRowNum(param1:int) : int
      {
         ...
         {
            _loc4_ = NUM_PORTS_PER_ROW_ARR[_loc3_];
            _loc2_ += _loc4_;
            if(param1 < _loc2_)
            {
               return _loc3_;
      ...
      private function convGlobalIndToRowInd(param1:int) : int
      {
         ...
         while(_loc4_ < _loc2_)
         {
            _loc5_ = NUM_PORTS_PER_ROW_ARR[_loc4_];
            _loc3_ -= _loc5_;
            _loc4_++;
         }
         return _loc3_;
      ...
      private function getNearestPortrait(param1:String, param2:int = -1, param3:int = -1) : CharacterPortrait
      {
         ...
         var _loc13_:int = this.PORT_VEC.length;
         var _loc14_:int = 0;
         while(_loc14_ < _loc13_)
         {
            if((_loc15_ = this.PORT_VEC[_loc14_]).parent)
            {
      ...
      {
         var _loc4_:int = param2.indexOf(param3);
         var _loc5_:int = 0;
