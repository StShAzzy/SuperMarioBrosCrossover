package com.smbc.characters
{
   ...
   public class Link extends Character
   {
      ...
      public static const NEVER_LOSE_UPGRADES:Array = [LINK_BOW,LINK_BOMB,LINK_QUIVER,LINK_BOMB_BAG];
      
      public static const RESTORABLE_UPGRADES:Array = [LINK_MAGIC_BOOMERANG,LINK_MAGIC_SWORD];
      ...
      public static const SKIN_ORDER:Array = [SKIN_LINK_NES,SKIN_LINK_SNES,SKIN_LINK_GB,SKIN_LINK_X1,SKIN_LINK_ATARI,SKIN_DARK_LINK_NES,SKIN_DARK_LINK_SNES,SKIN_DARK_LINK_GB,SKIN_ZELDA_NES,SKIN_ZELDA_SNES,SKIN_ZELDA_GB,SKIN_RALPH,SKIN_OLD_MAN_NES,SKIN_HERO_SNES,SKIN_HEROINE_SNES,SKIN_CHRISTINE,SKIN_ERDRICK,SKIN_ROTO,SKIN_LOTO,SKIN_BENJAMIN];
      ...
      override protected function mcReplacePrep(param1:MovieClip) : void
      {
         ...
         while(_loc4_ < param1.numChildren)
         {
            ...
            {
               if((_loc7_ = (_loc6_ = _loc5_ as MovieClip).totalFrames) == this.sword.totalFrames)
               {
                  _loc2_ = _loc6_;
               }
      ...
      override protected function getOnVine(param1:Vine) : void
      {
         ...
         if(this.flipTmr && this.flipTmr.running)
         {
            this.flipTmr.stop();
         }
      }
      ...
      override protected function setAmmo(param1:String, param2:int) : void
      {
         ...
         if(param1 == this.curSubWeaponName)
         {
      ...
      {
         if(!upgradeIsActive(MUSHROOM) && !(GameSettings.difficulty == Difficulties.VERY_EASY && STAT_MNGR.getObtainedUpgradesDct(charNum).length))
         {
            this.swordLevel = 1;
            damageAmt = DamageValue.LINK_SWORD_1;
         }
         else if(!upgradeIsActive(LINK_MAGIC_SWORD))
         {
            this.swordLevel = 2;
      ...
      override public function hitProj(param1:Projectile) : void
      {
         ...
         if(!upgradeIsActive(LINK_BLUE_RING) && !starPwr && param1 is LinkProjectile && (param1 as LinkProjectile).type == LinkProjectile.TYPE_BOMB)
         {
            takeDamage(param1);
         }
      }
      ...
      {
         if(cState == ST_ATTACK && ATK_DCT[param1])
         {
            return false;
         }
      ...
      override public function hitPickup(param1:Pickup, param2:Boolean = true) : void
      {
         ...
         var _loc3_:String = param1.mainType;
         if(_loc3_ == PickupInfo.MAIN_TYPE_UPGRADE)
         {
            switch(param1.type)
            {
               case MUSHROOM:
         ...
         else if(_loc3_ == PickupInfo.MAIN_TYPE_REGULAR)
         {
            switch(param1.type)
            {
               case LINK_BOMB_AMMO:
      ...
      override public function pressSpcBtn() : void
      {
         ...
         else if(hasEnoughAmmo(this.curSubWeaponName))
         {
            if(this.curSubWeaponName == SW_BOW)
            {
               level.addToLevel(new LinkProjectile(this,LinkProjectile.TYPE_ARROW));
               decAmmo(this.curSubWeaponName);
               this.attackStart(true);
            }
            else if(this.curSubWeaponName == SW_BOMB)
            {
               if(this.numProjectilesOfType(LinkProjectile.TYPE_BOMB) < MAX_BOMBS_ON_SCREEN)
               {
      ...
      private function numProjectilesOfType(param1:String) : int
      {
         ...
         for each(_loc3_ in level.PLAYER_PROJ_DCT)
         {
            ...
            {
               if((_loc4_ = _loc3_ as LinkProjectile).type == param1)
               {
                  _loc2_++;
               }
            }
      ...
      override protected function removeListeners() : void
      {
         ...
         if(this.atkTmr && this.atkTmr.hasEventListener(TimerEvent.TIMER_COMPLETE))
         {
