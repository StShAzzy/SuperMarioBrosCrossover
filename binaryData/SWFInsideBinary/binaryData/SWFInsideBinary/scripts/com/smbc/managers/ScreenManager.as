package com.smbc.managers
{
   ...
   public final class ScreenManager extends MainManager
   {
      ...
      private const CREDITS_SPEED_FAST:int = 400.0;
      
      private var speedUpCredits:Boolean;
      
      private const CREDITS_VISIBLE_END_Y:int = 64.0;
      
      private const CREDITS_VISIBLE_START_Y:int = 416.0;
      ...
      public function restartGame() : void
      {
         ...
         if(this.blackRect && this.blackRect.parent)
         {
            this.blackRect.parent.removeChild(this.blackRect);
         }
         if(this.blackRectBg && this.blackRectBg.parent)
         {
      ...
      public function startNewGameHandler() : void
      {
         ...
         this.createLevel(statMngr.curFullLevStr);
      }
      ...
      private function addTxtTmrHandler(param1:TimerEvent) : void
      {
         ...
         if(level.worldNum != statMngr.LAST_WORLD)
         {
            this.castleTxt.multiline = true;
            this.castleTxt.textBlockWidth = this.CASTLE_TXT_WIDTH;
            _loc3_ = GameTextMessages.ANOTHER_CASTLE;
            if((_loc4_ = statMngr.getCurrentBmc().namesArr) != null)
            {
               if((_loc5_ = _loc4_[BmdSkinCont.IND_NAME_ARR_PRINCESS]) != null)
               {
      ...
      private function moveCreditsLoopTmrHandler(param1:TimerEvent) : void
      {
         ...
         if(this.thankTxt && this.thankTxt.y + this.thankTxt.height < this.CREDITS_VISIBLE_END_Y)
         {
            game.removeChild(this.thankTxt);
            this.thankTxt = null;
         }
         else if(this.castleTxt && this.castleTxt.y + this.castleTxt.height < this.CREDITS_VISIBLE_END_Y)
         {
            game.removeChild(this.castleTxt);
            this.castleTxt = null;
         }
         else if(this.creditsTxt && this.creditsTxt.y + this.creditsTxt.height < this.CREDITS_VISIBLE_END_Y)
         {
      ...
      {
         level = new Level(statMngr.curFullLevStr,LevelDataManager.currentLevelData,this.newAreaStatsArr,statMngr.newLev);
         game.addChild(level);
      ...
      public function loadNewArea(param1:String, param2:String, param3:String, param4:Array) : void
      {
         ...
         this.createLevel(param1 + param3);
      }
      ...
      public function loadNewLevel(param1:String) : void
      {
         if(param1.length == 3)
         {
            param1 += "a";
